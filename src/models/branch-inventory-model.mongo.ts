...
export interface BranchInventoryModelDocument extends IBranchInventory, Document {
    _id: any;
}
...
const branchInventorySchema = new Schema<BranchInventoryModelDocument>(
    {
        branchId: { type: ObjectId as any, ref: branchTableName, required: true },
        productId: { type: ObjectId as any, ref: productTableName, required: true },
...
branchInventorySchema.index({ branchId: 1, productId: 1, variantId: 1 }, { unique: true });

const BranchInventoryModel = model<BranchInventoryModelDocument, IBranchInventoryModel>(
    branchInventoryTableName,
